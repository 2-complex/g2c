...
void Value::mimmic(const Value& v)
{
    ...
    if(type != v.type)
    {
        celloerror( "Attempt to set value to incongruous type.\n" );
        exit(0);
    }
...
void Value::applyToLocation(GLuint location) const
{
    ...
    {
        case NONE:
            celloerror( "Attempt to bind emtpy value to location." );
            exit(0);
        break;
...
float Value::getFloat() const
{
    if( type!=FLOAT )
    {
        celloerror( "Attempt to get float from value of incongruous type.\n" );
        exit(0);
    }
    GLfloat* t = data.ptr;
    return *t;
...
Vec2 Value::getVec2() const
{
    if( type!=VEC2 )
    {
        celloerror( "Attempt to get Vec2 from value of incongruous type.\n" );
        exit(0);
    }
    GLfloat* t = data.ptr;
    return Vec2(t[0], t[1]);
...
Vec3 Value::getVec3() const
{
    if( type!=VEC3 )
    {
        celloerror( "Attempt to get Vec3 from value of incongruous type.\n" );
        exit(0);
    }
    GLfloat* t = data.ptr;
...
Vec4 Value::getVec4() const
{
    if( type!=VEC4 )
    {
        celloerror( "Attempt to get Vec4 from value of incongruous type.\n" );
        exit(0);
    }
    GLfloat* t = data.ptr;
...
Mat2 Value::getMat2() const
{
    if( type!=MAT2 )
    {
        celloerror( "Attempt to get Mat2 from value of incongruous type.\n" );
        exit(0);
    }
    GLfloat* t = data.ptr;
    return Mat2(t[0], t[1],
...
Mat3 Value::getMat3() const
{
    if( type!=MAT3 )
    {
        celloerror( "Attempt to get Mat3 from value of mismatched type.\n" );
        exit(0);
    }
    GLfloat* t = data.ptr;
...
Mat4 Value::getMat4() const
{
    if( type!=MAT4 )
    {
        celloerror( "Attempt to get Mat4 from value of mismatched type.\n" );
        exit(0);
    }
    GLfloat* t = data.ptr;
...
const Texture* Value::getTexture() const
{
    if( type!=TEXTURE )
    {
        celloerror( "Attempt to get Texture from value of mismatched type.\n" );
        exit(0);
    }
    return data.texture;
}
...
void Effect::bindAttributeToField(const string& name, const Field& field) const
{
    if( !field.buffer )
    {
        celloerror( "Attempt to bind attributes to field with no buffer\n" );
        exit(0);
    }
    ...
    {
        celloerror( "Attempt to bind to non-existant attribute: %s\n", name.c_str() );
        exit(0);
    }
    ...
    {
        celloerror( "Attempt to draw with field size not 1 2 3 or 4.\n" );
        exit(0);
    }
...
void Effect::addUniform(const char* name)
{
    ...
    if(location<0)
    {
        celloerror( "location turned up negative for uniform: %s\n", name );
        exit(0);
    }
...
void Assumption::handleChild(const parse::Node* n)
{
    ...
    else if(!value->children.empty())
    {
        ...
        switch(v.size())
        {
            ...
            default:
                cellolog( "Value of %s of unusual size : %d\n", n_name.c_str(), (int)(v.size()) );
                exit(0);
            break;
        }
    }
...
void Geometry::bindAttributes(const Effect* effect)
{
    if(!effect)
    {
        celloerror( "Attributes bound to null effect\n" );
        exit(0);
    }
...
void Geometry::draw() const
{
    if( !indices )
    {
        celloerror( "Attempt to draw geometry with undefined indices.\n" );
        exit(0);
    }
...
void Shape::draw() const
{
    ...
    if(!geometry)
    {
        celloerror( "Shape drawn with no geometry.\n" );
        exit(0);
    }
    ...
    if(!effect)
    {
        celloerror( "Shape %s drawn with no effect.\n", name.c_str() );
        exit(0);
    }
...
Model& Model::add(Element* element, bool flagForDelete)
{
    ...
    {
        celloerror( "Element not inserted" );
        exit(0);
    }
...
void Model::resolveNames()
{
        ...
        itr++)
    {
        ...
        for(Assumption::iterator itr = assumption->begin(); itr!=assumption->end(); itr++)
        {
            ...
            if( textureName!="" )
            {
                ...
                {
                    celloerror( "texture not found %s\n", textureName.c_str() );
                    exit(0);
                }
                else
                    value = mitr->second;
    ...
    for(map<string, Geometry*>::iterator itr = geometries.begin(); itr!=geometries.end(); itr++)
    {
        ...
        if(indicesName!="")
        {
            ...
            {
                celloerror( "indexbuffer not found %s\n", indicesName.c_str() );
                exit(0);
            }
            else
    ...
    for(map<string, Shape*>::iterator itr = shapes.begin(); itr!=shapes.end(); itr++)
    {
        ...
        if(geometryName!="")
        {
            ...
            {
                celloerror( "geometry not found %s\n", geometryName.c_str() );
                exit(0);
            }
            else
            ...
            itr++)
        {
            ...
            {
                celloerror( "assumption not found %s\n", assumptionName.c_str() );
                exit(0);
            }
            else
