...
void MacBank::initTextureWithCGImage(Texture2D* texture, CGImageRef image)
{
    ...
    if(((width-1)&width) != 0 || ((height-1)&height) != 0)
        cellolog( "WARNING: texture dimensions not a power of two: %s.\n",
                texture->name.c_str() );
...
void MacFileSystemBank::initBitmapWithPath(Bitmap* bitmap, const char* path)
{
    ...
    {
        celloerror( "Image source failed for %s\n", path );
...
void MacBank::initSoundWithPath(Sound* sound, const char* path)
{
    ...
    else
    {
        ...
        {
            celloerror("OpenAL error = 0x%x loading audio file: %s\n", error, fullpath.c_str());
            exit(0);
        }
                                  ...
                                  ALsizei* outSampleRate) const
{
    ...
    if(err)
    {
        celloerror("getOpenALAudioData: ExtAudioFileOpenURL FAILED, Error = %d\n", (int)err);
        exit(0);
    }
    ...
    if(err)
    {
        celloerror("getOpenALAudioData: ExtAudioFileGetProperty"
                 "(kExtAudioFileProperty_FileDataFormat) FAILED, Error = %d\n", (int)err);
        exit(0);
    }
    if (theFileFormat.mChannelsPerFrame > 2)
    {
        celloerror("getOpenALAudioData - Unsupported Format, channel count = %d.\n",
    ...
    if(err)
    {
        celloerror("getOpenALAudioData: ExtAudioFileSetProperty"
    ...
    if (theData)
    {
        ...
        else 
        {
            ...
            celloerror("getOpenALAudioData: ExtAudioFileRead FAILED, Error = %d\n", (int)err);
            exit(0);
        }    
    }
