...
int OpenSLPlayer::createContext()
{
    ...
    if( SL_RESULT_SUCCESS != result )
    {
        celloerror("OpenSL Failed to get create engine.\n");
        exit(0);
    }
    ...
    if( SL_RESULT_SUCCESS != result )
    {
        celloerror("OpenSL Failed to realize engine object.\n");
        exit(0);
    }
    ...
    if( SL_RESULT_SUCCESS != result )
    {
        celloerror("OpenSL Failed to get interface engine.\n");
        exit(0);
    }
    ...
    if( SL_RESULT_SUCCESS != result )
    {
        celloerror("OpenSL Failed to create output mix.\n");
        exit(0);
    }
    ...
    if( SL_RESULT_SUCCESS != result )
    {
        celloerror("Failed to initialize OpenSL.\n");
        exit(0);
    }
...
int OpenSLPlayer::createSource()
{
    ...
    if( SL_RESULT_SUCCESS != result )
    {
        celloerror("OpenSL Failed to initialize OpenSL audio player.\n");
        exit(0);
    }
...
void OpenSLPlayer::destroySource(int index)
{
    ...
    {
        celloerror("OpenSL Attempt to destroy non-existant source.");
        exit(0);
    }
...
void OpenSLPlayer::destroySound(int index)
{
    ...
    if( itr == audios.end() )
    {
        celloerror("OpenSL Attempt to destroy audio that doesn't exist. index=%d\n", index);
        exit(0);
    }
...
void OpenSLPlayer::makeContextCurrent(int index)
{
    ...
    if( itr == contextInfos.end() )
    {
        celloerror("OpenSL Attempt to make context current that doesn't exist. index=%d\n", index);
        exit(0);
    }
    ...
    const uint8_t* data)
{
    ...
    if( itr == audios.end() )
    {
        celloerror("OpenSL Attempt to load into non-existant audio buffer. index=%d\n", index);
        exit(0);
    }
...
void OpenSLPlayer::stopSource(int index)
{
    ...
    if( SL_RESULT_SUCCESS != result )
        cellolog("source failed to stop");
}
...
void OpenSLPlayer::SourceInfo::realize()
{
    ...
    if( SL_RESULT_SUCCESS != result )
    {
        celloerror("OpenSL Failed to realize player.");
        exit(0);
    }
    ...
    if( SL_RESULT_SUCCESS != result )
    {
        celloerror("OpenSL Failed to get player interface.");
        exit(0);
    }
    ...
    if( SL_RESULT_SUCCESS != result )
    {
        celloerror("OpenSL Failed register callback.");
        exit(0);
    }
    ...
    if( SL_RESULT_SUCCESS != result )
    {
        celloerror("OpenSL Failed to get effect send interface.");
        exit(0);
    }
    ...
    if( SL_RESULT_SUCCESS != result )
    {
        celloerror("OpenSL Failed to get volume control interface.");
        exit(0);
    }
    ...
    if( SL_RESULT_SUCCESS != result )
    {
        celloerror("OpenSL Failed to set player state to PLAYING.");
        exit(0);
    }
}
