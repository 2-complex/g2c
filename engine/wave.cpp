...
void checkTag(const char* tag, const char* exemplar)
{
    if( !(tag[0] == exemplar[0] &&
          ...
          tag[3] == exemplar[3] ) )
    {
        celloerror( "Wave parser expected tag '%c%c%c%c', found '%c%c%c%c'.\n",
...
void Wave::initWithData(const uint8_t* data, size_t size)
{
    ...
    {
        celloerror( "Wav data too small for riff header.\n" );
        exit(0);
    }
    ...
    {
        celloerror( "Wav data too small both riff header and wav header."
            "  Cannot read.\n" );
        exit(0);
    }
    ...
    {
        cellolog( "Size of data encoded in wav exceeds size of data given." );
        exit(0);
    }
}
